...
No lint or test scripts are defined. Typecheck with `npm run typecheck` (`tsc -b`, TypeScript Project References) — it covers all three bundles and is the closest thing to a test gate.

**Type-checking is split across four leaf tsconfigs** (`tsconfig.main.json` / `tsconfig.preload.json` / `tsconfig.renderer.json` / `tsconfig.recorder-dom.json`, all `extends`-ing `tsconfig.base.json`), wired together by a solution-style root `tsconfig.json` (`{ "files": [], "references": [...] }`) that `tsc -b` builds in one command. This exists so `lib: DOM` — needed by renderer, and by nothing else in main/preload — can't silently leak: main and preload run under `lib: ["ES2020"]` with no `window`/`document`, so a main-process file that accidentally references a browser global fails to compile instead of type-checking successfully by accident. The one part of main that *does* need DOM types is `src/main/playwright/browserScripts/` (`captureShared.ts` + `codegenCapture.ts` — see below), which is why those two files live in their own subdirectory with its own `tsconfig.recorder-dom.json`, referenced only by `tsconfig.main.json`. `tsc -b` writes declaration-only output (`.d.ts` + `.tsbuildinfo`, no `.js`) to the gitignored `.tsbuild/` — this is purely for cross-project type resolution; it has no effect on the real build below, which esbuild/electron-vite performs by transpiling each file directly and never reads `lib`/`references`/`composite`.
...
Singletons `browserController` and `recorder` are module-level in `ipcHandlers.ts`; the `Replayer` is deliberately **handler-local**, since holding it module-level would retain a finished run's `Page` and session vars. Two further module-level slots exist purely so a run can be cancelled — `activeReplaySignal` (a `CancelSignal`) and `activeRun` (`{ cancelled, kill }`) — and both hold only a **switch, never the object**, and are nulled in their handler's `finally`. Recording **always relaunches the browser** (Playwright's `_enableRecorder` can only be called once per context).
...
**Renderer → Main (35):** `RECORDING_START`, `RECORDING_STOP`, `REPLAY_TO_NODE`, `REPLAY_CANCEL` (stop the in-flight replay — interrupts mid-action, leaves the browser open), `FLOW_SAVE` (accepts `touch?: boolean`), `FLOW_LOAD`, `FLOW_LIST`, `FLOW_DELETE`, `FLOW_GET` (one flow JSON by ID), `FLOW_CHECK_CYCLE` (validate adding a callFlow won't create a circular reference — recursively walks the sub-flow's callFlow graph), `EXPORT_SCRIPTS`, `RUN_TESTS`, `TEST_CANCEL` (kill the in-flight test run *or* browser install — both stream into the same modal), `SHOW_REPORT` (bundled CLI `show-report`; kills any process on port 9323 first), `PROJECT_SAVE`, `PROJECT_LOAD`, `PROJECT_LIST`, `PROJECT_DELETE`, `PICK_FILES` (native open dialog; copies picks into `fixtures/` and returns their stored paths), `NORMALIZE_PATHS` (rewrite absolute paths as workspace-relative), `WORKSPACE_GET` (also reports `hasChromium` — there is no separate browser-check channel), `WORKSPACE_PICK` (native folder dialog → validate → scaffold → open), `WORKSPACE_SET` (open a remembered path), `WORKSPACE_FORGET`, `WORKSPACE_REVEAL` (`shell.openPath`), `BROWSER_INSTALL` (`playwright install chromium`, output over `TEST_OUTPUT`), `VAULT_STATUS`, `VAULT_SETUP`, `VAULT_UNLOCK`, `VAULT_LOCK`, `VAULT_CHANGE_PASSPHRASE`, `SECRET_ENCRYPT`, `SECRET_REVEAL`, `SECRETS_FILE_WRITE`, `APP_ERRORS_DRAIN` (collect failures main reported before the renderer subscribed — see the error-channel section)

There is **no browser lifecycle channel** (`BROWSER_LAUNCH` / `BROWSER_CLOSE`) and **no assertion-pick or locator-pick channel**: the browser is launched implicitly by `RECORDING_START` / `REPLAY_TO_NODE`, and both pickers now run entirely in-browser.

Replay *can* be cancelled (`REPLAY_CANCEL`), and cancelling **does not close the browser** — the Replayer races every Playwright call against a stop signal instead, so ⏹ lands mid-action while the page stays up for inspection. The **branch-recording silent replay is cancelled through `RECORDING_STOP`**, not `REPLAY_CANCEL` — see "Cancelling a recording start" below.

**Main → Renderer (12):** `ACTION_CAPTURED`, `ACTION_UPDATED` (retro-patch fields of an already-emitted action — e.g. stamping `opensPage` when the popup arrives late), `ACTION_REMOVED` (un-record a node — the click that opened a file chooser), `REPLAY_NODE_START`, `REPLAY_NODE_COMPLETE`, `REPLAY_FINISHED`, `REPLAY_ERROR`, `REPLAY_CANCELLED` (the user stopped it — distinct from `REPLAY_FINISHED` "reached the target", and from `REPLAY_ERROR`, which raises a toast and would make a deliberate action look like a failure), `TEST_OUTPUT`, `TEST_FINISHED` (carries `cancelled`), `WORKSPACE_RELOAD` (window regained focus — the workspace may have changed on disk), `APP_ERROR` (a failure main needs the user to see)
...
**Cancellation.** A run carries a `ReplaySession` — `{ pages, signal }` — handed verbatim to every nested Replayer, so a cancel raised anywhere unwinds the whole tree. `suppressedPages` is deliberately **not** in the session: it stays per-Replayer so an inner call-flow run can only release its own file-chooser listener. `private race<T>(op)` wraps every Playwright await (and the pace delay, via `sleep()`), because Playwright exposes no abort signal — a locator sitting on its 30 s timeout cannot be interrupted any other way. The honest cost: **the losing operation is not stopped**, it just stops being awaited, so a click may still land seconds after the UI says the run stopped. That is accepted because the browser is deliberately left open on cancel. Two things are *not* raced: `browserController.launch()` (killing a half-started Chromium leaves a zombie profile dir, so a ⏹ during launch is honoured only once launch completes — the one known latency exception) and anything synchronous. A cancel throws the `ReplayCancelledError` sentinel, which the loop uses to **skip `onNodeComplete`** so an interrupted node is never painted red; the renderer repaints every still-`running` node amber on `REPLAY_CANCELLED` — for a cancelled sub-flow that is *two* nodes, the deepest one and its parent `callFlow`.

`Replayer` constructor: `(page, baseURL = '', ctx: ResolutionContext = {}, session?: ReplaySession)` — the whole resolution context arrives as one object, normalized so `ctx.profileVars` / `ctx.envVars` are never undefined. The **project environment variable** `domain` (`ctx.envVars['domain']`, trailing slash stripped) drives goto URL origin substitution — if a goto URL's origin matches `baseURL`'s origin, it is replaced with that domain. `ctx.activeProfileId` + `ctx.activeEnvironmentId` let the replayer resolve `subFlowProfileMapping` and `envValues` on `callFlow` nodes at any nesting depth; `ctx.activeProjectId` gates env-var resolution to the active project. `executeCallFlow()` loads the sub-flow, resolves its profile, and builds a nested `Replayer` with a **derived context** (`{...this.ctx, profileVars: subProfileVars, activeProfileId: subProfile?.id, envVars: subFlowEnvVars}` — the last one same-project-gated), then merges captured session vars back up.
...
4. On exit, `TEST_FINISHED` fires (carrying `cancelled`). The HTML report is opened separately via `SHOW_REPORT` (`showReport()`), which kills any process on port 9323 first.
5. `TEST_CANCEL` → `RunHandle.cancel` → `killTree(pid)`. `runPlaywright` returns a **handle**, not a bare promise, so a child cannot be spawned without also handing back a way to kill it — the spawn used to be local to the promise executor, which is exactly why runs were uncancellable. `child.kill()` is not enough on win32: we run our own binary as Node, which spawns the Playwright runner, which spawns workers and the browser, and Windows has no process groups — so `taskkill /F /T` walks the tree (POSIX falls back to `pkill -P` + SIGKILL, best-effort). `SpawnResult.cancelled` is set **by the killer**, never inferred from the exit code: a taskkill'd child reports exit 1, indistinguishable from a real test failure. `BROWSER_INSTALL` shares the same `activeRun` slot, since both stream into the same modal.
...
- **Changing the passphrase is all-or-nothing** (`security/recrypt.ts`). Per-file atomicity is not enough here: rewriting every stored secret is a *batch*, and the original per-file `load → rewrite → save` loop left the already-written files under the NEW key while the verifier still expected the OLD one — that batch then opened with neither passphrase. `recryptWorkspace` therefore runs in three phases: **(1)** load and re-encrypt entirely in memory, which is where the likeliest failure lands (one envelope that won't decrypt) with nothing yet written; **(2)** copy every file it is about to overwrite into `.flowtest/vault-backup/<ts>/` (gitignored, last 3 generations kept) — the only thing that survives a crash; **(3)** `stageJson` every file beside its destination, *then* `commitStaged` them all, so disk-full and permission failures happen before anything is replaced and the commit is nothing but renames. A rename failing part way restores every original from the backup. The one residual is a rollback that itself fails, which is reported as a toast naming the backup path. Files with no ciphertext are skipped entirely — no rewrite, no `updatedAt` bump.
- **A successful change pushes `WORKSPACE_RELOAD`.** Disk now holds new-key ciphertext, but the renderer is still holding the OLD ciphertext for the open flow, and its next autosave would write that straight back over it. The window never lost focus, so the existing focus-reload reconciliation would not have run on its own. This is also why phase 1 stamps `updatedAt` on every rewritten flow: the reload gate compares against `persistence.ts`'s ledger, so the old `touch: false` write made the gate structurally incapable of firing (A16).
...
- **UI**: 🔐 toggles private (🔒 stays reserved for the `domain` lock); `common/SecretValue.tsx` masks read-only displays with a 👁 that fetches one plaintext over `SECRET_REVEAL`. Private values are also excluded from `EnvVarPickerPopover`'s search corpus, which would otherwise be an oracle for them.
...
- **Where the fields live** — `PropertyPanel` (six `useState` fields, re-synced by a `useEffect` keyed on **`selectedNodeId` only**, never on the node object: a node drag or a recorder `ACTION_UPDATED` replaces that object and would otherwise wipe in-progress typing). Both variable tables (`ProfileVarTable` / `ProjectEnvVarModal`) hold their draft in **`hooks/useDraftRows.ts`**, the one state machine behind both: `rows` + `error`, `setCell` / `addRow` / `removeRow` / `toggleSecret`, the reload `useEffect` keyed on `flow:profile:env` / `project:env`, and `prepareValues` (validate → gate the vault → resolve every row's on-disk value). Each table keeps its own `toEditRows`, its `commitXxx` call and its post-save reload.
...
- **Row identity** — table rows carry `_rid` (stable client id; also the key of `ProfileVarTable`'s value-input caret map) plus `_origIndex` (profile vars) / `_origKey` (project env vars), which `commitProfileVars` / `commitProjectEnvVars` need to tell added rows from edited ones and to carry other environments' values across a rename. `_storedValue` + `_dirty` back the private-value round trip: an untouched private row writes its original ciphertext back verbatim rather than re-encrypting. One 儲存 commits the whole table.
- **Validation runs in `prepareValues`** — empty key / duplicate key (both tables), then an `extraCheck` callback for per-table rules ("`domain` must survive" for project env vars); failures set `error`, return `null`, and abort the write.
- **`src/renderer/stores/confirmStore.ts` + `components/common/ConfirmDialog.tsx`** — `confirm()` raises an app-styled dialog from `<ConfirmHost />` (mounted once in `App.tsx`, `zIndex.confirm` — near the top of the scale — so it can appear above modals). `notify()` is the one-OK-button variant that replaced the last native `alert()`s. Both `window.confirm` and `alert` now have zero call sites. Danger dialogs put Enter on 取消.
...
### Reporting failures (two channels, one rule)

In an Electron app with DevTools closed by default, a `console.error` is indistinguishable from nothing happening at all — which is what the user saw when a save failed, a replay died, or a dragged edge was refused. There are now two channels, and **which one to use is decided by whether the user is standing there waiting for the result, not by severity**:

- **Modal — `notify()` / `confirm()` (`stores/confirmStore.ts`)** for a failure in something the user just pressed and is waiting on: saving a node, starting a recording, running the tests. A failed autosave is more serious than a failed export, and still must not use this: it lands mid-drag.
- **Toast — `reportError()` (`stores/errorStore.ts`, rendered by `components/common/ToastHost.tsx`)** for background or asynchronous failures: `persistFlow` / `persistProject`, `REPLAY_ERROR`, a node's `REPLAY_NODE_COMPLETE` error, the debounced drag save, a refused `connectNodes`.

`reportError` **dedupes on title + detail**, bumping a `×N` counter instead of pushing a second entry — the failures most worth reporting are the ones that repeat (an unwritable workspace fails on every single autosave), and without this the fix would itself be an outage. Toasts never expire on a timer, for the same reason the channel exists at all; the stack is capped at 4. `reportError` still logs to the console, so DevTools loses nothing.

- **`errorStore` must stay a leaf module** — it imports no other store. `stores/persistence.ts` is the single owner of every disk write and is used by *both* `flowStore` and `projectStore`, so it can only reach for something that cannot close an import cycle. That is also why persistence failures had no other way to surface: they are fire-and-forget, so no call site is left awaiting them.
- **Node failure reasons** live in `flowStore.replayErrors` (keyed by node id, set through `setReplayStatus`'s third argument). Like `replayStatus` it is **not** part of `Flow`, so the undo subscription — which snapshots `currentFlow` — cannot see it. `ActionNode` renders it as a native `title` tooltip, so the reason outlives the toast.
- `zIndex.toast` (4500) sits **above** `zIndex.confirm`: background failures keep arriving while a dialog is open, and a bottom-right corner never covers a centred card.

#### Main's half — `src/main/errorChannel.ts`

Storage and the recorder have no `BrowserWindow`, and threading one through every `FlowStorage` call is not an option, so the window lives in a **leaf module** — same shape and same rationale as `errorStore` on the renderer side. `reportToUser(title, detail?, tone?)` sends `APP_ERROR`; the renderer turns it straight into a toast. **Main never raises a modal**: none of these are questions the user can answer in a dialog, and a toast does not expire.

- **The buffer's gate is "has the renderer drained yet", not "is the window set yet".** `setErrorSink(win)` runs inside `registerIpcHandlers`, still well before React has mounted and subscribed, so a live send in that window would be dropped — and startup failures are worse than that: `main/index.ts` awaits `loadSettings()` *before* `createWindow()`, so they happen with no window at all. Everything is buffered until the renderer calls `APP_ERRORS_DRAIN`, which it does in `usePlaywrightEvents` **immediately after** subscribing to `APP_ERROR`. Subscribe-then-drain is what leaves no gap: earlier reports are in the buffer, later ones arrive live.
- **`list()` reports once per scan, not once per file.** A directory of unreadable files would otherwise raise a toast each and blow past the 4-deep cap, so `FlowStorage.list` / `ProjectStorage.list` collect the failures and emit one message naming them.
- **`load()` still returns `Flow | null` for both "missing" and "unreadable"** — callers are unchanged — but only the first is normal, so the second is reported. Known limitation: `reloadFromDisk` reads a null as "deleted out from under us" and closes the open flow, and it still does. The user now at least learns why instead of watching the flow vanish.
- `delete()` reports anything that is not `ENOENT`: the renderer drops the row either way, so a failed unlink used to mean the flow silently reappeared on the next reload.
- The remaining `catch {}` blocks in main are all deliberate and carry a comment saying so (`browserController.close`, the vault's keychain fallback, `replayer`'s locator fallback, the first-run settings read).
...
- **`Modal.tsx`** — two variants. `compact` (padding 24, text title) and `panel` (fixed `width`, bordered header with ✕, `maxHeight: 80vh`, scrolling body, bordered footer). Both close on Escape (a window-level capturing listener) and on backdrop click. Four modals opt out via `closeOnBackdrop` / `closeOnEscape`, each for a stated reason: the two variable-table editors would silently discard the whole in-progress table, `VaultModal` would drop a half-typed passphrase, and `TestOutputModal` stays up for the duration of a run because it is where the ⏹ 中止 button lives and it must survive the kill to report 已中止 and whatever the runner printed on its way out. **This is not dirty tracking and must not become it** — see the editing-model note above.
...
- **External changes**: `win.on('focus')` → `WORKSPACE_RELOAD` → `reloadFromDisk()`, which re-reads both lists and reconciles the open flow and the open project. Skipped while recording/replaying.
  - **The open flow** is replaced only when the disk copy is **not the one this renderer last wrote** — `persistence.ts` keeps a per-flow ledger of the `updatedAt` that `FLOW_SAVE` resolved with, and `reloadFromDisk` compares against that. The obvious test, "is the disk copy newer than mine", was true after *every* local edit and so reloaded on every focus, taking the undo history, the selection and the active profile with it (`setCurrentFlow` clears all three): the renderer stamps its in-memory copy, `FlowStorage.save` then stamps its own a few ms later, and that later stamp never travelled back. Making the stamp round-trip is what lets the gate mean what it always intended. **No ledger entry means the flow has not been saved by this renderer, so the disk copy wins** — reloading then costs nothing, since there is no history or selection behind it yet. A flow, unlike a project, cannot use content comparison: it has legitimately unsaved in-memory state (the drag debounce, and `persistFlow` being fire-and-forget), so "differs from disk" cannot tell our own pending write from someone else's edit. The cost of using a stamp instead is that an external write which leaves `updatedAt` untouched stays invisible.
  - **The open project** is replaced whenever its content differs — `projectSignature()` compares `name` / `environments` / `envVars`, and there is deliberately no `updatedAt` gate. `updatedAt` is a field *inside* the JSON, not a filesystem mtime: a hand edit or a `git pull` leaves it untouched and a checkout to an older branch moves it backwards, so a `>` test cannot see an external change at all. It is also stale by construction — `ProjectStorage.save` stamps the new timestamp onto main's own copy, which never travels back — so including it would report a phantom difference on every focus after any local edit. Dropping the gate is safe for a project and not for a flow because a project has **no unsaved in-memory state** (every write goes straight through `persistProject`), and the one draft that does exist, the env-var table, survives because `useDraftRows` is keyed on `projectId:envId` rather than on the project object's identity.
  - **A project that vanished** from disk clears the context (`setProjectContext(null, null)`) instead of leaving the environment selector and every env-var reference resolving against a ghost. Both this and the flow branch inherit `load()`'s known limitation: a corrupted file is also a `null`, so it is treated as a deletion (with a toast explaining why).

### Storage

- Flows: `<workspace>/flows/{flowId}.json`. `FlowStorage.list()` also computes each flow's `refCount` by scanning all callFlow nodes. `save(flow, { touch: false })` writes without bumping `updatedAt` — used by the debounced drag save, since repositioning is not a content change (drag coordinates are also rounded) and would otherwise conflict in git for nothing. `save()` **returns the `updatedAt` that landed on disk**, which `FLOW_SAVE` passes back to the renderer; the focus reload depends on it (see "External changes").
...
| `src/main/security/recrypt.ts` | `recryptWorkspace()` — the all-or-nothing rewrite behind `changePassphrase`: rewrite in memory → back originals up under `.flowtest/vault-backup/<ts>/` → stage every file → commit as one batch of renames, restoring from the backup if that fails. Stamps `updatedAt` on rewritten flows so the focus reload can see them |
...
| `src/main/playwright/runner.ts` | Bundled Playwright CLI: `resolvePlaywrightCli()` (asar-unpacked path + version + `nodePath`), `runPlaywright()` (spawns `process.execPath` with `ELECTRON_RUN_AS_NODE` + `NODE_PATH` + pinned HTML report dir; returns a `RunHandle` so the child can be killed), `killTree()` (win32 `taskkill /F /T`, POSIX `pkill -P` + SIGKILL) |
...
| `src/main/storage/atomicWrite.ts` | The one owner of `.tmp` + `rename` JSON writes: `writeJsonAtomic` (used by both storage classes) plus the `stageJson` / `commitStaged` / `discardStaged` split that lets `security/recrypt.ts` make a whole batch atomic, not just each file |
| `src/main/storage/flowStorage.ts` | Flow CRUD; `list()` computes `refCount`; sorts by updatedAt; `save(flow, { touch })`, which returns the stamp it wrote |
...
| `src/preload/index.ts` | contextBridge — exposes `window.electronAPI`, pinned with `satisfies ElectronAPI`; the 11 Main→Renderer `onX` wrappers all come from one generic `subscribe<T>(channel)` |
...
| `src/renderer/stores/persistence.ts` | `persistFlow` / `persistProject` — the single point of ownership for disk writes, and the **only** caller of `electronAPI.saveFlow` (`opts.label` names the failure toast for callers that had their own message). Also owns the `lastWrittenStamp` ledger the focus reload gates on — it lives here rather than on the in-memory `Flow` precisely because this module is a leaf: writing it back into the store would close a `flowStore → persistence → flowStore` cycle. A leaf module importing no store, which is what lets both stores use it with no import cycle |
...
| `src/renderer/components/common/ConfirmDialog.tsx` | `ConfirmHost` — renders queued confirm requests (near the top of the zIndex scale, Enter defaults to 取消 on danger) |
| `src/main/errorChannel.ts` | Main's half of the error channel: `reportToUser()` / `describeError()` / `isNotFound()`, plus `setErrorSink()` + `drainBufferedErrors()`. A leaf module (electron's type + shared channels only) so storage can reach it; buffers until the renderer drains, since startup failures precede the window |
| `src/renderer/stores/errorStore.ts` | The non-blocking half of the error channel: `reportError()` (deduped on title+detail, `×N` counter, stack capped at 4, never auto-expires) + `formatError()`. A leaf module — imports no other store, which is what lets `persistence.ts` use it |
| `src/renderer/components/common/ToastHost.tsx` | Renders those toasts bottom-right at `zIndex.toast` — above `confirm`, so a background failure is still visible with a dialog open |
...
| `src/renderer/components/Toolbar/Toolbar.tsx` | Action bar: workspace name (📂 click to reveal) + switch (⇄, blocked while recording/replaying), new-flow, undo/redo, record/stop, relayout, export, run-tests, replay-speed, environment selector (🌐), profile selector (⚙), status pills (the ⟳ 重播中 pill carries the ⏹ 停止重播 button — it needs no disabled state because `isReplaying` already gates its existence); new-flow dialog (歸類至專案 + 流程名稱, no 目標URL) |
| `src/renderer/components/Toolbar/TestOutputModal.tsx` | Streams live `TEST_OUTPUT` lines during `RUN_TESTS`; hosts the ⏹ 中止 button (disabled while the kill is in flight, since Playwright keeps printing as it dies) |
| `src/renderer/components/Canvas/FlowCanvas.tsx` | ReactFlow canvas (Background / Controls / MiniMap): node/edge derivation (incl. groups), drag-reposition with debounced save (settled by an effect keyed on `currentFlow.id` **and** `flowEpoch`: persisted when the flow changed, discarded when the same flow was replaced by its disk copy), connect/disconnect, multi-select, node + pane context menus, modals (CallFlow / ExtractSubflow / GroupName / AddNode); one-time layout materialization |
| `src/renderer/components/Canvas/ActionNode.tsx` | Custom node: type icon/color, description, selector, replay-status border (green pass / red fail / **amber interrupted** — `running` has no branch, so amber is what makes a cancel visible at all), page-nav border, callFlow profile badge |
...
| `src/renderer/hooks/useDraftRows.ts` | The draft state machine both variable tables run on: `rows`/`error`, `setCell` (`_dirty` only on the value column), `addRow`/`removeRow`, `toggleSecret`, the reload effect keyed on `reloadKey`, and `prepareValues` (validation + vault gate + the private-value round trip). Also `newRowId()` and `submitOnEnter()` |
| `src/renderer/components/common/varTable.tsx` | The table pieces both editors had byte-identical copies of: `cellInputStyle`, `SECRET_FIELD_BORDER`, `varHeaderStyle`/`varRowStyle` grid factories, `SecretToggleButton` (🔐), `DeleteRowButton` (🗑), `TableEmptyState`. Deliberately holds no column definitions — the two tables' columns genuinely differ |
| `src/renderer/components/ProfileEditor/ProfileEditorModal.tsx` | 環境配置管理 shell — two columns and one piece of state (`selectedProfileId`). Mounts `ProfileVarTable` with `key={profile.id}`, so switching profiles remounts the table and drops the caret map with it |
| `src/renderer/components/ProfileEditor/ProfileList.tsx` | Left column: profile list + add / rename (commits on ✓/Enter only, never on blur) / duplicate / delete. Owns its inline add and rename buffers |
| `src/renderer/components/ProfileEditor/ProfileVarTable.tsx` | Right column: one profile's variable table (key synced across profiles; value/description per-profile) committed by 儲存 via `commitProfileVars`, the 環境值 selector, and the value-input caret map (`valueInputRefs` + `lastValueCaret`) that `EnvVarPickerPopover` inserts through |
| `src/renderer/components/ProfileEditor/EnvVarPickerPopover.tsx` | 🌐 專案環境變數 trigger + `position: fixed` popover (search, 🔒/🔐 badges, value preview). Reports only which key was picked; `onPick` returns `'insert' \| 'copy'` to drive the badge. Private values are excluded from the search corpus — matching them would make the filter an oracle |
| `src/renderer/components/ProjectEnvVar/ProjectEnvVarModal.tsx` | Project-level env-var editor — local table (one key per row, value per selected environment) committed by 儲存 via `commitProjectEnvVars`; `domain` row is key-locked, cannot be made private, and is non-deletable (🔒) |
| `src/renderer/components/ProjectEnvVar/EnvironmentToolbar.tsx` | The pinned strip above that table: environment `<select>` + add / rename / duplicate / delete (the last environment can't be removed). Owns its inline add and rename buffers |
...
| `src/renderer/hooks/usePlaywrightEvents.ts` | IPC event subscriptions: ACTION_CAPTURED, ACTION_UPDATED, ACTION_REMOVED, REPLAY_NODE_*, REPLAY_FINISHED/ERROR/CANCELLED, WORKSPACE_RELOAD; owns `reloadFromDisk()` |
| `src/renderer/hooks/usePlaywright.ts` | IPC invocation wrappers: startRecording (navigates to the active env's `domain`, persists it as `flow.baseURL`), startBranchRecording (sets `isRecording` **before** the await, so ⏹ exists during the silent replay; clears it again on `started: false`), stopRecording, replayToNode (builds env-aware profileVars + envVars), cancelReplay (deliberately does **not** clear `isReplaying` itself — `REPLAY_CANCELLED` owns that, as `REPLAY_FINISHED` does) |
