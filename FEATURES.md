...
- Zustand store 保存 `past[]` / `future[]` 快照，上限 50 步。
- 訂閱式記錄：任何替換 `currentFlow` 物件的編輯都自動快照，並在時光旅行中、拖曳中、切換流程、錄製 / 重播進行中自動跳過。
- 快捷鍵 `Ctrl/Cmd+Z`（復原）與 `Ctrl/Cmd+Shift+Z`（重做），在輸入框中打字或錄製 / 重播期間自動停用。
- 工具列另有 **↶ 復原** / **↷ 重做** 按鈕。
...
| **編輯** | 拖曳定位 · 拖曳連接 / 斷開 · 多選 · 中斷連線 · 刪除（含 / 不含子樹） · 自動樹狀佈局 · 一鍵整理 · 視覺群組（折疊 / 展開 / 解散） · 復原重做（50 步 + 快捷鍵） |
