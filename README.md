...
> 把瀏覽器操作錄成**可分支的視覺化流程圖**，再一鍵產生標準的 Playwright `.spec.ts` 測試套件。

FlowTest 是一套 Electron 桌面應用。它和 Playwright 官方 codegen 一樣錄製使用者的點擊、輸入與斷言，但錄出來的不是一支線性腳本，而是一棵**節點樹**：每條「根 → 葉」的路徑就是一個測試案例，共用的前段步驟只需錄一次。

核心功能是**分支錄製**：在畫布上任選一個已錄製的節點，FlowTest 會在背景把瀏覽器重播到那一步，再從那個狀態繼續錄製。原本要把 30 步重錄一次才能測另一條路徑，現在只需要錄最後分岔的 5 步。

除了錄製，FlowTest 也是完整的流程編輯器：拖曳排版、連線與斷線、多選、復原 / 重做、視覺群組、子流程抽取與嵌入、多環境變數，以及錄製無法表達時可手寫 Playwright 程式的「程式碼節點」。
...
| 特點 | 說明 |
|------|------|
| **分支錄製** | 從任一節點背景重播後繼續錄製，不必每條路徑都從頭錄。 |
| **樹狀流程，而非線性腳本** | 一個流程可有多條分支，每條「根 → 葉」路徑匯出為一個 `test()`。 |
| **官方等級的 Locator** | 直接使用 `playwright-core` 內的 `InjectedScript` 產生 locator，錄出 `getByRole('button', { name: '登入' })` 這類語意化定位，而非脆弱的 CSS 路徑。 |
| **產出標準 Playwright 專案** | 匯出的 `.spec.ts` 不依賴 FlowTest，可以直接 `npx playwright test`、進版控、接 CI。 |
| **測試資料跟著你的 repo 走** | 所有資料存在你指定的「工作區」資料夾中，放進自己的 repo 就能跟原始碼一起 commit、開 PR。 |
| **內建測試執行器** | App 自帶 `@playwright/test`，以 Electron 內建的 Node 執行；使用者的機器不需要安裝 Node，專案也不需要是 Node 專案。 |
| **多層環境管理** | 專案環境（DEV / UAT / PRD）＋流程環境配置＋區域變數＋內建變數，切換環境即換掉整組網域與變數值。 |
| **私密資料加密** | 密碼等敏感值以 AES-256-GCM 加密後存檔；匯出的腳本透過 `process.env` 讀取，明文不會出現在 spec 中。 |
| **子流程重用** | 登入、選單導覽等共用步驟抽成子流程，支援 N 層巢狀與逐層的配置對應。 |
| **瀏覽器內互動 UI** | 斷言選取與 Locator 選擇直接出現在被錄製的瀏覽器內，錄製節奏不中斷。 |
