...
test.describe('flow7', () => {
  ...
  test('導航到 https://esd.genesys-tech.com/ → 導航到 https://esd.genesys-tech.com/SignOff/SignOffFirst', async ({ page }) => {
    ...
    await test.step('點擊「簽核管理」', async () => {
      await page.getByRole("button", { name: "簽核管理", exact: true }).click();
    });

    await test.step('點擊「簽核申請❐」', async () => {
      await page.getByRole("button", { name: "簽核申請", exact: false }).click();
    });

    await test.step('導航到 https://esd.genesys-tech.com/SignOff/SignOffFirst', async () => {
      await page.goto('https://esd.genesys-tech.com/SignOff/SignOffFirst');
    });
  });
