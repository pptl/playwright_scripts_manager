...
/**
 * The `window.electronAPI` surface, in `src/shared/` so preload and renderer read the same
 * declaration instead of maintaining two hand-written copies.
 *
 * `src/preload/index.ts` pins its object literal to this with `satisfies ElectronAPI`, which
 * catches a missing/extra method or a changed parameter list at compile time. It cannot catch
 * a wrong *return* type: `ipcRenderer.invoke` returns `Promise<any>`, which is assignable to
 * anything declared here — those still have to agree with `ipcHandlers.ts` by inspection.
 *
